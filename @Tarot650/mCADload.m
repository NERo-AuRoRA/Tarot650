function mCADload(uav)
% Load Tarot 650-Sport 3D CAD model

uav.pCAD.obj{1} = loadawobj('tarotBody.obj');
uav.pCAD.mtl{1} = loadawmtl('tarotBody.mtl');

uav.pCAD.obj{2} = loadawobj('tarotPaFD.obj');
uav.pCAD.mtl{2} = loadawmtl('tarotPaFD.mtl');

uav.pCAD.obj{3} = loadawobj('tarotPaFE.obj');
uav.pCAD.mtl{3} = loadawmtl('tarotPaFE.mtl');


% Flags
uav.pCAD.flagLoaded  = 1;  % 3D image loaded
uav.pCAD.flagCreated = 0;  % 3D image created using Matlab Patch function
uav.pCAD.flagLines   = 1;  % Illustrate lines between and surface

uav.pCAD.flagpPlot = 1;


%------------------------------------------------
num = length(uav.pCAD.obj);

for i = 1:num

    vExt = minmax(uav.pCAD.obj{i}.v);
    mExt = mean(vExt,2);

    for ii = 1:3
        uav.pCAD.obj{i}.v(ii,:) = (uav.pCAD.obj{i}.v(ii,:) - mExt(ii))/900;
    end

    
    uav.pCAD.obj{i}.v = [1 0 0; 0 0 -1; 0 1 0]*uav.pCAD.obj{i}.v;
    uav.pCAD.obj{i}.v = [0 1 0; -1 0 0; 0 0 1]*uav.pCAD.obj{i}.v;
    
    if i == 1      
        uav.pCAD.obj{i}.v(3,:) = uav.pCAD.obj{i}.v(3,:) - (min(uav.pCAD.obj{i}.v(3,:))+0.0)*ones(1,length(uav.pCAD.obj{i}.v(3,:)));

    elseif i == 2
        uav.pCAD.obj{i}.v(3,:) = uav.pCAD.obj{i}.v(3,:) - (min(uav.pCAD.obj{i}.v(3,:)))*ones(1,length(uav.pCAD.obj{i}.v(3,:)));
